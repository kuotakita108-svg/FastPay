...
export default function ProfilePage(){
  ...
  if(isMarketing)return <main className="mobile-app profile-page marketing-kuotakita-profile">
    ...
    <section className="profile-menu marketing-profile-menu"><header><span>MENU AKUN</span></header>{marketingMenus.map(([title,Icon,tone])=><button type="button" key={title}><i className={`tone-${tone}`}><Icon/></i><div><strong>{title}</strong></div><ChevronRight/></button>)}</section>
