...
export default function CreditApplicationsPage() {
  ...
  const creditAuditFindings = sortedItems.flatMap(item => {
    ...
    if (item.status === 'Disetujui' && item.paymentStatus !== 'Lunas' && !item.dueAt) return [{...base, area:'Jadwal', level:'Belum Lengkap', title:'Tanggal jatuh tempo belum diisi', note:'Kredit aktif belum memiliki jadwal pelunasan.', impact:'Tagihan tidak muncul di menu Jatuh Tempo', amount:outstanding, action:'Isi Tanggal', target:'detail'}]
    if (item.agentAccessStatus === 'suspended' && !String(item.agentAccessReason || '').trim()) return [{...base, area:'Akses', level:'Belum Lengkap', title:'Alasan suspend belum dicatat', note:'Akses agent sudah dihentikan tanpa keterangan.', impact:'Riwayat tindakan Operator tidak lengkap', amount:outstanding, action:'Lengkapi Alasan', target:'suspend'}]
    return []
  })
    ...
    <section className={`panel credit-review-panel ${isStandaloneDetail ? 'detail-mode' : ''} ${isMarketing ? 'marketing-review' : ''} ${(isOperator || isAdmin) ? 'analyst-review operator-review' : ''}`}>
      ...
      {(isOperator || isAdmin) && view === 'laporan' && <section className="credit-audit-workspace">
        ...
        <section className="credit-audit-panel">
          <header><div><span>DAFTAR TUGAS OPERATOR</span><h3>{creditAuditFindings.length ? `${creditAuditFindings.length} data harus dibereskan` : 'Semua data sudah beres'}</h3><p>Masalah paling penting berada paling atas. Selesaikan satu per satu sampai daftar kosong.</p></div><strong>{auditRows.length}<small>Tampil</small></strong></header>
          <div className="credit-audit-tools"><label><Search/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Cari agent, toko, atau masalah..."/></label><label><UserCheck/><select value={reportMarketing} onChange={event=>setReportMarketing(event.target.value)}><option>Semua Marketing</option>{auditMarketingNames.map(name=><option key={name}>{name}</option>)}</select></label><div>{['Semua','Pembayaran','Jadwal','Saldo','Akses'].map(name=><button type="button" className={reportFilter===name?'active':''} onClick={()=>setReportFilter(name)} key={name}>{name}</button>)}</div><button type="button" onClick={()=>{setQuery('');setReportFilter('Semua');setReportMarketing('Semua Marketing')}}>Reset</button></div>
