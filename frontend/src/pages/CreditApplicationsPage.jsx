...
export default function CreditApplicationsPage() {
  ...
  const [fieldTaskDraft, setFieldTaskDraft] = useState({applicationId:'', type:'Kunjungan ulang', note:'', dueAt:''})
  ...
  const createFieldTask = event => {
    ...
    setFieldTaskDraft({applicationId:'', type:'Kunjungan ulang', note:'', dueAt:''})
  ...
  return <>
    <section className={`panel credit-review-panel ${isStandaloneDetail ? 'detail-mode' : ''} ${isMarketing ? 'marketing-review' : ''} ${(isOperator || isAdmin) ? 'analyst-review operator-review' : ''}`}>
      ...
      {(isOperator || isAdmin) && view === 'laporan' && <section className="operator-field-assignment">
        ...
        <form className="field-assignment-form" onSubmit={createFieldTask}><header><div><span>BUAT TUGAS BARU</span><h3>Instruksi kunjungan agent</h3></div><PlusCircle/></header><div className="field-assignment-fields"><label><span>Agent yang ditangani</span><select value={fieldTaskDraft.applicationId} onChange={event=>setFieldTaskDraft(current=>({...current,applicationId:event.target.value}))}><option value="">Pilih agent</option>{sortedItems.filter(item=>item.marketingOwnerId || item.marketingOwnerName || item.marketingName).map(item=><option key={item.id} value={item.id}>{item.form.agentName || item.userName} — {item.form.storeName || item.id}</option>)}</select></label><label><span>Jenis pekerjaan</span><select value={fieldTaskDraft.type} onChange={event=>setFieldTaskDraft(current=>({...current,type:event.target.value}))}><option>Kunjungan ulang</option><option>Perbaikan dokumen</option><option>Konfirmasi data toko</option><option>Penagihan lapangan</option><option>Ambil bukti pembayaran</option><option>Hubungi agent</option></select></label><label><span>Batas waktu</span><input type="datetime-local" value={fieldTaskDraft.dueAt} onChange={event=>setFieldTaskDraft(current=>({...current,dueAt:event.target.value}))}/></label><label className="assignment-note"><span>Instruksi Operator</span><textarea rows="3" value={fieldTaskDraft.note} onChange={event=>setFieldTaskDraft(current=>({...current,note:event.target.value}))} placeholder="Tuliskan hasil yang harus dibawa Marketing, misalnya: foto ulang KTP yang jelas dan konfirmasi alamat toko..."/></label></div><button type="submit"><ArrowRight/>Kirim ke Agenda Marketing</button></form>
