...
const viewInfo = {
  ...
  laporan: {label: 'Perbaikan Data Kredit', title: 'Bereskan data yang belum lengkap', desc: 'Selesaikan data kredit yang menghambat penagihan, pelunasan, atau keamanan akun agent.'},
...
export default function CreditApplicationsPage() {
  ...
  const creditAuditFindings = sortedItems.flatMap(item => {
    ...
    if (issued > 0 && paid > issued) return [{...base, level:'Mendesak', code:'NOMINAL-LEBIH', title:'Uang masuk lebih besar dari pinjaman', note:`Ada selisih ${rupiah(paid - issued)}. Cocokkan nominal sebelum kredit ditutup.`, amount:paid, action:'Cocokkan Nominal', target:'detail'}]
    if (item.paymentStatus === 'Lunas' && outstanding > 0) return [{...base, level:'Mendesak', code:'SALDO-BELUM-NOL', title:'Sudah lunas tetapi saldo belum nol', note:`Saldo kredit masih tercatat ${rupiah(outstanding)} dan perlu diperbaiki.`, amount:outstanding, action:'Perbaiki Saldo', target:'detail'}]
    if (item.paymentStatus !== 'Lunas' && issued > 0 && paid >= issued) return [{...base, level:'Perlu Diperiksa', code:'PELUNASAN-BELUM-TUTUP', title:'Pembayaran sudah penuh, status belum lunas', note:'Periksa bukti lalu tutup kredit agar saldo agent kembali benar.', amount:paid, action:'Selesaikan Pelunasan', target:'pelunasan'}]
    if (item.paymentStatus === 'Menunggu verifikasi pembayaran') return [{...base, level:'Perlu Diperiksa', code:'BUKTI-MENUNGGU', title:'Bukti pembayaran belum diperiksa', note:'Cocokkan nominal, waktu, dan bukti transfer yang dikirim agent.', amount:paid, action:'Periksa Bukti', target:'pelunasan'}]
    if (item.status === 'Disetujui' && item.paymentStatus !== 'Lunas' && !item.dueAt) return [{...base, level:'Belum Lengkap', code:'JATUH-TEMPO-KOSONG', title:'Tanggal jatuh tempo belum diisi', note:'Isi tanggal agar tagihan muncul otomatis di menu Jatuh Tempo.', amount:outstanding, action:'Isi Tanggal', target:'detail'}]
    if (item.agentAccessStatus === 'suspended' && !String(item.agentAccessReason || '').trim()) return [{...base, level:'Belum Lengkap', code:'ALASAN-SUSPEND-KOSONG', title:'Alasan suspend belum dicatat', note:'Lengkapi alasan agar riwayat pengamanan akun jelas.', amount:outstanding, action:'Lengkapi Alasan', target:'suspend'}]
    return []
  })
  ...
  return <>
    <section className={`panel credit-review-panel ${isStandaloneDetail ? 'detail-mode' : ''} ${isMarketing ? 'marketing-review' : ''} ${(isOperator || isAdmin) ? 'analyst-review operator-review' : ''}`}>
      ...
      {(isOperator || isAdmin) && view === 'laporan' && <section className="credit-audit-workspace">
        <header className="credit-audit-hero"><div><span>PERBAIKAN DATA KREDIT</span><h2>Bereskan data yang menghambat proses</h2><p>Hanya data bermasalah yang tampil. Setelah diperbaiki, data otomatis hilang dari daftar ini.</p></div><ShieldCheck/></header>
        <div className="credit-audit-summary">
          <article><span>Perlu diperbaiki</span><strong>{creditAuditFindings.length}</strong><small>Semua pekerjaan</small></article>
          <article className="critical"><span>Mendesak</span><strong>{creditAuditFindings.filter(row=>row.level==='Mendesak').length}</strong><small>Berisiko salah saldo</small></article>
          <article className="review"><span>Perlu diperiksa</span><strong>{creditAuditFindings.filter(row=>row.level==='Perlu Diperiksa').length}</strong><small>Butuh keputusan</small></article>
          <article className="clean"><span>Sudah benar</span><strong>{Math.max(0, sortedItems.length - new Set(creditAuditFindings.map(row=>row.item.id)).size)}</strong><small>Tidak perlu tindakan</small></article>
        </div>
        <section className="credit-audit-panel">
          <header><div><span>DAFTAR YANG HARUS DIBERESKAN</span><h3>Masalah dan tindakan penyelesaiannya</h3><p>Pilih tombol pada setiap baris untuk langsung menuju tempat perbaikannya.</p></div><strong>{auditRows.length}<small>Pekerjaan</small></strong></header>
          <div className="credit-audit-tools"><label><Search/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Cari agent, toko, masalah, atau ID..."/></label><label><UserCheck/><select value={reportMarketing} onChange={event=>setReportMarketing(event.target.value)}><option>Semua Marketing</option>{auditMarketingNames.map(name=><option key={name}>{name}</option>)}</select></label><div>{['Semua','Mendesak','Perlu Diperiksa','Belum Lengkap'].map(name=><button type="button" className={reportFilter===name?'active':''} onClick={()=>setReportFilter(name)} key={name}>{name}</button>)}</div><button type="button" onClick={()=>{setQuery('');setReportFilter('Semua');setReportMarketing('Semua Marketing')}}>Reset</button></div>
          <div className="credit-audit-table">
            {!!auditRows.length && <div className="credit-audit-head"><span>Prioritas</span><span>Masalah</span><span>Agent / Toko</span><span>Marketing</span><span>Nominal</span><span>Penyelesaian</span></div>}
            ...
            {!auditRows.length && <div className="credit-audit-empty"><ShieldCheck/><b>Semua data pada bagian ini sudah benar</b><span>Tidak ada pekerjaan yang perlu diselesaikan untuk filter yang dipilih.</span></div>}
          </div>
        </section>
      </section>}
