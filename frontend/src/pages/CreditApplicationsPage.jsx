      ...
      {(isMarketing || isOperator || isAdmin) && view === 'peminjam' && <section className="borrower-directory-panel mentoring-directory">
        ...
        <div className="directory-agent-list">
          ...
          {directoryGroups.length ? directoryGroups.map(group => {const row=group.rows[0]; const item=row.item; const stage=mentoringStage(item); const profile=creditProfile(items,item); const payable=group.rows.find(({item: candidate}) => candidate.status === 'Disetujui' && candidate.paymentStatus !== 'Lunas')?.item; const activityAt=item.updatedAt || item.marketingSurveyAt || item.createdAt; return <article className="mentored-agent-row" key={group.key}>
            ...
            <small className="mentored-agent-updated">{dateTime(activityAt)}</small><span className="mentored-agent-progress"><em title={`Kelengkapan ${row.score.percent}%`}><i style={{width: `${row.score.percent}%`}}/></em><small>{row.score.percent}% lengkap</small></span>
