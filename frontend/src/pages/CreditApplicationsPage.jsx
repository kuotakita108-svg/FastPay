  ...
  return <>
    <section className={`panel credit-review-panel ${isStandaloneDetail ? 'detail-mode' : ''} ${isMarketing ? 'marketing-review' : ''} ${(isOperator || isAdmin) ? 'analyst-review operator-review' : ''}`}>
      ...
      {(isOperator || isAdmin) && view === 'laporan' && <section className="operator-field-assignment">
        ...
        <div className="field-assignment-summary"><article><small>Tugas aktif</small><strong>{operatorFieldTasks.filter(row=>row.task.status!=='Selesai').length}</strong><span>Masih dikerjakan</span></article><article><small>Selesai</small><strong>{operatorFieldTasks.filter(row=>row.task.status==='Selesai').length}</strong><span>Sudah dikonfirmasi Marketing</span></article><article><small>Marketing terlibat</small><strong>{new Set(operatorFieldTasks.map(row=>row.item.marketingOwnerId || row.item.marketingOwnerName).filter(Boolean)).size}</strong><span>Memiliki penugasan</span></article></div>
