...
const viewInfo = {
  ...
  laporan: {label: 'Penugasan Lapangan', title: 'Kirim tugas kepada Marketing', desc: 'Buat instruksi kunjungan, perbaikan dokumen, konfirmasi toko, atau penagihan untuk agent tertentu.'},
  ...
  'transaksi-agent': {label: 'Transaksi Agent', title: 'Aktivitas transaksi Agent', desc: 'Pantau transaksi dari akun Agent. Aktivitas pengguna reguler tetap dipisahkan di menu Transaksi Pengguna.'},
...
export default function CreditApplicationsPage() {
  ...
  const createFieldTask = event => {
    event.preventDefault()
    const item = sortedItems.find(row => row.id === fieldTaskDraft.applicationId)
    if (!item) return setOperatorMessage('Pilih agent yang akan menerima penugasan.')
    if ((item.marketingOwnerName || item.marketingName || '') !== fieldTaskDraft.marketing) return setOperatorMessage('Agent tidak sesuai dengan Marketing yang dipilih. Pilih ulang agent binaan.')
    if (!fieldTaskDraft.note.trim()) return setOperatorMessage('Instruksi tugas wajib ditulis dengan jelas.')
    if (!item.marketingOwnerId && !item.marketingOwnerName && !item.marketingName) return setOperatorMessage('Agent ini belum memiliki Marketing penanggung jawab.')
    const task = {id:`TASK-${Date.now()}`, type:fieldTaskDraft.type, note:fieldTaskDraft.note.trim(), dueAt:fieldTaskDraft.dueAt, priority:fieldTaskDraft.dueAt && new Date(fieldTaskDraft.dueAt).getTime() <= Date.now()+86400000 ? 'Mendesak' : 'Normal', status:'Ditugaskan', createdAt:new Date().toISOString(), createdBy:reviewerName(user)}
    saveApplication(item, {operatorTasks:[task, ...(Array.isArray(item.operatorTasks) ? item.operatorTasks : [])]})
    setFieldTaskDraft({marketing:'', applicationId:'', type:'Verifikasi ulang data agent', note:'', dueAt:''})
    setOperatorMessage(`Tugas untuk ${item.form.agentName || item.userName} sudah dikirim ke Marketing.`)
    refresh()
  }
  const completeFieldTask = (item, task) => {
    const operatorTasks = (Array.isArray(item.operatorTasks) ? item.operatorTasks : []).map(row => row.id === task.id ? {...row, status:'Selesai', completedAt:new Date().toISOString(), completedBy:reviewerName(user)} : row)
    saveApplication(item, {operatorTasks})
    refresh()
  }
  ...
  const fieldAgenda = marketingOwnedItems.flatMap(item => {
    const assigned = (Array.isArray(item.operatorTasks) ? item.operatorTasks : []).filter(task => task.status !== 'Selesai').map(task => ({item, kind: task.type, note: task.note, operatorTask: task, priority: task.priority === 'Mendesak' ? 0 : 1}))
    if (item.status === 'Perlu Revisi Marketing') return [...assigned, {item, kind: 'Revisi berkas', note: 'Lengkapi kembali data atau foto yang dikembalikan Operator', priority: 1}]
    if (['Menunggu verifikasi marketing', 'Sedang diverifikasi marketing'].includes(item.status)) return [...assigned, {item, kind: 'Survei agent', note: marketingReadiness(item).readyForAnalysis ? 'Berkas siap dikirim kepada Operator' : 'Lengkapi data dan empat foto survei', priority: 2}]
    if (item.status === 'Disetujui' && item.paymentStatus === 'Menunggu penagihan offline') return [...assigned, {item, kind: 'Kunjungan pembayaran', note: 'Konfirmasi jadwal dan catat bukti penerimaan', priority: 3}]
    return assigned
  }).sort((a, b) => a.priority - b.priority || new Date(a.item.updatedAt || a.item.createdAt) - new Date(b.item.updatedAt || b.item.createdAt))
  const operatorFieldTasks = sortedItems.flatMap(item => (Array.isArray(item.operatorTasks) ? item.operatorTasks : []).map(task => ({item, task}))).sort((a,b) => new Date(b.task.createdAt || 0) - new Date(a.task.createdAt || 0))
  const fieldTaskMarketingOptions = [...new Set(sortedItems.map(item => item.marketingOwnerName || item.marketingName || '').filter(Boolean))].sort((a,b)=>a.localeCompare(b,'id'))
    ...
    <section className={`panel credit-review-panel ${isStandaloneDetail ? 'detail-mode' : ''} ${isMarketing ? 'marketing-review' : ''} ${(isOperator || isAdmin) ? 'analyst-review operator-review' : ''} ${isOwner ? 'owner-review' : ''}`}>
      ...
      {(isMarketing || isAdmin) && !isOwner && view === 'overview' && <section className="marketing-workspace">
        ...
        <div className="marketing-quick-actions" aria-label="Aksi cepat marketing">
          ...
          <button type="button" onClick={() => offlineCollectionQueue[0] ? goToView('angsuran-detail', offlineCollectionQueue[0].id, 'Disetujui') : goToView('peminjam')}><HandCoins/><span><b>Pelunasan offline</b><small>{offlineCollectionQueue.length ? `${offlineCollectionQueue.length} agent perlu dikunjungi` : 'Tidak ada penagihan tertunda'}</small></span><strong>→</strong></button>
      ...
      {(isMarketing || isOperator || isAdmin) && view === 'peminjam' && <section className={`borrower-directory-panel mentoring-directory ${isOwner ? 'owner-monitor-list' : ''}`}>
        <header><div><span>{isMarketing ? 'AGEN BINAAN' : isOwner ? 'PANTAUAN PORTOFOLIO' : 'KREDIT AKTIF'}</span><h2>{isMarketing ? 'Pantau setiap agent binaan' : isOwner ? 'Kredit agent seluruh jaringan' : 'Kontrol kredit agent'}</h2><p>{isMarketing ? 'Lihat tahap pengajuan, kelengkapan survei, kredit aktif, dan pelunasan dalam satu daftar.' : isOwner ? 'Ringkasan kredit, penanggung jawab, tagihan, dan status akhir tanpa mengubah pekerjaan staf.' : 'Periksa kredit berjalan, atur limit secara langsung, dan pantau pelunasan setiap agent.'}</p></div><strong className="directory-total">{directoryGroups.length}<small>{isMarketing ? 'Agent binaan' : 'Agent kredit'}</small></strong></header>
        ...
        <div className="directory-agent-list" ref={directoryScrollRef}>
          ...
          {directoryGroups.length ? directoryGroups.map((group,index) => {const row=group.rows[0]; const item=row.item; const stage=mentoringStage(item); const profile=creditProfile(items,item); const payable=group.rows.find(({item: candidate}) => candidate.status === 'Disetujui' && candidate.paymentStatus !== 'Lunas')?.item; const paymentRow=payable ? paymentRows(payable).find(candidate => !candidate.paid) : null; const activityAt=item.updatedAt || item.marketingSurveyAt || item.createdAt; const marketingName=item.marketingOwnerName || item.marketingName || 'Belum ditugaskan'; return <article className={`mentored-agent-row ${!isMarketing ? 'has-marketing' : ''}`} key={group.key}>
            ...
            <div className="mentoring-actions">{isOwner ? <><button type="button" onClick={() => goToView('detail', item.id, 'Semua')}><Eye/>Lihat Detail</button><button type="button" onClick={() => printApplication(item)}><Printer/>Cetak</button></> : isMarketing ? <>
              <button type="button" onClick={() => goToView('detail', item.id, 'Semua')}><Eye/>{stage === 'Survei' ? 'Lanjut Survei' : 'Detail'}</button>
              <button type="button" disabled={!item.form.whatsapp} onClick={() => window.open(`https://wa.me/${String(item.form.whatsapp || '').replace(/\D/g, '').replace(/^0/, '62')}?text=${encodeURIComponent(`Halo ${item.form.agentName || item.userName}, saya dari Marketing KuotaKita ingin menindaklanjuti pengajuan kredit ${item.id}.`)}`, '_blank', 'noopener,noreferrer')}><PhoneCall/>Hubungi</button>
              <button type="button" disabled={!payable || !paymentRow} onClick={() => {if(!payable || !paymentRow)return; setPaymentTarget({item:payable,row:paymentRow}); setPaymentMethod(''); setPaymentProof(null)}}><Banknote/>Bayar</button>
              <button type="button" disabled={stage !== 'Aktif' && stage !== 'Lunas'} onClick={() => saveMarketingRecommendation(item)}><TrendingUp/>Rek. Limit</button>
            </> : <>
              <button type="button" onClick={() => goToView('detail', item.id, 'Semua')}><Eye/>Detail</button>
              <button type="button" onClick={() => {setOperatorDrafts(current => ({...current,[item.id]:String(profile.limit)}));setLimitTarget(item)}}><Gauge/>Atur Limit</button>
              <button type="button" className={item.agentAccessStatus === 'suspended' ? 'restore' : 'suspend'} onClick={() => setAgentAccess(item, item.agentAccessStatus !== 'suspended')}><Ban/>{item.agentAccessStatus === 'suspended' ? 'Aktifkan' : 'Hentikan'}</button>
              <button type="button" onClick={() => printApplication(item)}><Printer/>Cetak</button>
            </>}</div>
      ...
      {(isMarketing || isAdmin) && view === 'agenda' && <section className="field-agenda-panel">
        ...
        <div className="field-agenda-list">{fieldAgenda.length ? fieldAgenda.map(({item, kind, note, operatorTask}) => <article key={`${item.id}-${operatorTask?.id || kind}`}><time>{operatorTask?.dueAt ? `Batas ${dateTime(operatorTask.dueAt)}` : dateTime(item.updatedAt || item.createdAt)}</time><div><b>{item.form.agentName || item.userName}</b><small>{item.form.storeName || 'Toko belum diisi'} · {item.form.whatsapp || item.id}</small></div><em>{kind}</em><p>{note}</p><nav><button type="button" onClick={() => goToView(kind === 'Kunjungan pembayaran' ? 'angsuran-detail' : 'detail', item.id, item.status)}><Eye/>Buka agent</button>{operatorTask && <button type="button" className="complete-field-task" onClick={()=>completeFieldTask(item,operatorTask)}><Check/>Tandai selesai</button>}</nav></article>) : <div className="field-agenda-empty"><CalendarDays/><b>Agenda lapangan sudah tertangani</b><span>Tugas baru akan muncul otomatis saat ada agent yang perlu disurvei, direvisi, atau dikunjungi.</span></div>}</div>
      </section>}
      {(isOperator || isAdmin) && view === 'pelunasan' && <section className={`analyst-payment-proof-panel ${isOwner ? 'owner-monitor-list' : ''}`}>
        <header className="payment-proof-heading">
          <i><FileCheck2/></i>
          <div><span>{isOwner ? 'AUDIT PELUNASAN' : 'VERIFIKASI &amp; ARSIP'}</span><h2>Pelunasan Kredit Agent</h2><p>{isOwner ? 'Pantau bukti yang menunggu Operator dan seluruh pelunasan yang sudah tercatat.' : 'Periksa bukti pembayaran yang baru masuk dan simpan seluruh kredit yang sudah lunas dalam satu arsip resmi.'}</p></div>
        </header>
        <div className="payment-proof-list">
          ...
          {pendingProofRows.map(({item, payment, proof}) => <article key={`pending-${item.id}`} className="payment-proof-card pending">
            ...
            {!isOwner && <button type="button" className="approve" disabled={!proof?.dataUrl} onClick={() => confirmFullPayment(item)}><CheckCircle2/>{proof?.dataUrl ? 'Verifikasi pelunasan' : 'Menunggu bukti'}</button>}
          </article>)}
      ...
      {(isOperator || isAdmin) && view === 'laporan' && <section className="operator-field-assignment">
        <header><div><span>PENUGASAN LAPANGAN</span><h2>Kirim instruksi kepada Marketing</h2><p>Pilih agent, tentukan pekerjaan, lalu kirim. Tugas hanya masuk ke Agenda Lapangan Marketing penanggung jawab agent tersebut.</p></div><ClipboardCheck/></header>
        <form className="field-assignment-form" onSubmit={createFieldTask}><header><div><span>BUAT TUGAS BARU</span><h3>Instruksi pekerjaan lapangan</h3><p>Pilih Marketing terlebih dahulu agar pilihan agent tidak tercampur dengan binaan Marketing lain.</p></div><PlusCircle/></header><div className="field-assignment-fields"><label><span>Marketing penanggung jawab</span><select value={fieldTaskDraft.marketing} onChange={event=>setFieldTaskDraft(current=>({...current,marketing:event.target.value,applicationId:''}))}><option value="">Pilih Marketing</option>{fieldTaskMarketingOptions.map(name=><option key={name} value={name}>{name}</option>)}</select><small>{fieldTaskDraft.marketing ? `${fieldTaskAgentOptions.length} agent binaan tersedia` : 'Agent akan muncul setelah Marketing dipilih'}</small></label><label><span>Agent binaan</span><select disabled={!fieldTaskDraft.marketing} value={fieldTaskDraft.applicationId} onChange={event=>setFieldTaskDraft(current=>({...current,applicationId:event.target.value}))}><option value="">{fieldTaskDraft.marketing?'Pilih agent binaan':'Pilih Marketing dahulu'}</option>{fieldTaskAgentOptions.map(item=><option key={item.id} value={item.id}>{item.form.agentName || item.userName} — {item.form.storeName || item.id}</option>)}</select></label><label><span>Batas penyelesaian</span><input type="datetime-local" value={fieldTaskDraft.dueAt} onChange={event=>setFieldTaskDraft(current=>({...current,dueAt:event.target.value}))}/></label><label className="assignment-type"><span>Jenis pekerjaan</span><select value={fieldTaskDraft.type} onChange={event=>setFieldTaskDraft(current=>({...current,type:event.target.value}))}><option>Verifikasi ulang data agent</option><option>Foto ulang KTP yang lebih jelas</option><option>Foto ulang toko dan lokasi usaha</option><option>Konfirmasi alamat dan kondisi toko</option><option>Kunjungan penagihan kredit</option><option>Ambil bukti pelunasan</option><option>Konfirmasi agent melalui WhatsApp</option><option>Investigasi agent tidak aktif</option></select><small>{fieldTaskDraft.type==='Kunjungan penagihan kredit'?'Pastikan nominal tagihan dan hasil kunjungan dicatat.':fieldTaskDraft.type==='Ambil bukti pelunasan'?'Marketing wajib membawa bukti pembayaran yang dapat diperiksa.':fieldTaskDraft.type.includes('Foto ulang')?'Foto harus terang, utuh, dan sesuai objek yang diminta.':'Tuliskan hasil akhir yang harus dilaporkan Marketing.'}</small></label><label className="assignment-note"><span>Instruksi dan hasil yang wajib dibawa</span><textarea rows="3" value={fieldTaskDraft.note} onChange={event=>setFieldTaskDraft(current=>({...current,note:event.target.value}))} placeholder="Contoh: kunjungi toko, cocokkan alamat, ambil foto tampak depan, lalu laporkan apakah usaha masih aktif..."/></label></div><button type="submit" disabled={!fieldTaskDraft.marketing || !fieldTaskDraft.applicationId || !fieldTaskDraft.note.trim()}><ArrowRight/>Kirim ke Agenda {fieldTaskDraft.marketing || 'Marketing'}</button></form>
        {operatorMessage && <output className="operator-feedback" aria-live="polite">{operatorMessage}</output>}
        <section className="field-assignment-history"><header><div><span>PEMANTAUAN TUGAS</span><h3>Riwayat pekerjaan lapangan</h3><p>Operator dapat melihat tugas yang masih berjalan dan yang sudah diselesaikan Marketing.</p></div><strong>{operatorFieldTasks.length}<small>Total</small></strong></header><div>{operatorFieldTasks.length ? operatorFieldTasks.map(({item,task})=><article className={task.status==='Selesai'?'is-complete':''} key={task.id}><div><b>{item.form.agentName || item.userName}</b><small>{item.form.storeName || item.id} · {item.marketingOwnerName || item.marketingName || 'Belum ditugaskan'}</small></div><em>{task.type}</em><p>{task.note}</p><time>{task.dueAt ? `Batas ${dateTime(task.dueAt)}` : `Dibuat ${dateTime(task.createdAt)}`}</time><strong>{task.status}</strong></article>) : <div className="field-assignment-empty"><ClipboardCheck/><b>Belum ada penugasan lapangan</b><span>Buat tugas pertama untuk menghubungkan pekerjaan Operator dengan Agenda Marketing.</span></div>}</div></section>
      </section>}
      ...
      {(isOperator || isAdmin) && view === 'kinerja-marketing' && <section className={`marketing-performance-workspace ${isOwner ? 'owner-monitor-list' : ''}`}>
        <header><div><span>{isOwner ? 'PANTAUAN TIM' : 'KONTROL TIM LAPANGAN'}</span><h2>Kinerja Marketing</h2><p>{isOwner ? 'Lihat produktivitas dan kualitas portofolio setiap marketing tanpa mengambil alih pekerjaan Operator.' : 'Pantau produktivitas, penyelesaian survei, hasil keputusan, nilai kredit aktif, dan risiko setiap marketing.'}</p></div><ShieldCheck/></header>
      ...
      {items.length === 0 ? <div className="credit-review-empty"><ShieldCheck/><strong>Belum ada pengajuan</strong><span>Pengajuan kredit saldo dari aplikasi agent akan tampil di sini.</span></div> : visibleItems.length === 0 ? <div className="credit-review-empty"><Search/><strong>Data tidak ditemukan</strong><span>Coba ubah kata pencarian atau filter status.</span></div> : operatorTableMode ? <section className="operator-queue-table">
              ...
              return <tr key={item.id}>
                ...
                <td>
                  <div className="operator-row-actions">
                    ...
                    {!isOwner && (item.status === 'Disetujui' || item.agentAccessStatus === 'suspended') && <button type="button" className={item.agentAccessStatus === 'suspended' ? 'restore' : 'suspend'} onClick={() => setAgentAccess(item, item.agentAccessStatus !== 'suspended')}><Ban/>{item.agentAccessStatus === 'suspended' ? 'Aktifkan' : 'Hentikan'}</button>}
                  </div>
                </td>
              </tr>
            })}</tbody>
        ...
        {pagedItems.map(item => {
          ...
          const canOperatorSign = !isOwner && !done && item.status === 'Menunggu keputusan operator' && analysis.ready && !operatorSigned && (isOperator || isAdmin)
          const canApprove = !isOwner && item.status === 'Menunggu keputusan operator' && !done && operatorSigned && analysis.ready && (isOperator || isAdmin)
          const canReject = !isOwner && item.status === 'Menunggu keputusan operator' && !done && operatorSigned && Boolean(decisionNote.trim()) && (isOperator || isAdmin)
          return <article className={`credit-review-card status-${item.status.toLowerCase().replaceAll(' ', '-')} ${expanded ? 'expanded' : ''}`} key={item.id}>
            ...
            <footer>
              ...
              {expanded && !isOwner && (isMarketing || isAdmin) && score.percent < 100 && <span className="meeting-required"><AlertCircle/>Lengkapi data dan empat dokumen survei oleh marketing</span>}
                {expanded && !isOwner && (isMarketing || isAdmin) && item.status === 'Menunggu verifikasi marketing' && <button type="button" className="sign" onClick={() => startMarketingSurvey(item)}><Camera/>Mulai Survei</button>}
                {expanded && !isOwner && (isMarketing || isAdmin) && score.percent === 100 && !meetingSelfieReady && <span className="meeting-required"><Camera/>Ambil selfie pertemuan bersama agent</span>}
                {expanded && !isOwner && (isMarketing || isAdmin) && readiness.readyForAnalysis && item.status !== 'Menunggu keputusan operator' && <button type="button" className="approve" onClick={() => forwardToOperator(item)}><ArrowRight/>Kirim ke Operator</button>}
            ...
            {expanded && <section className="credit-borrower-detail">
              ...
              {isDetail && !isOwner && (isOperator || isAdmin) && <div className="credit-detail-block operator-control-block">
              ...
              {isDetail && !isOwner && (isOperator || isAdmin) && (() => {
                const analysis = analystReadiness(item)
              ...
              {isDetail && !isOwner && (isMarketing || isAdmin) && <div className="credit-detail-block field-survey-panel">
              ...
              {isInstallmentDetail && <div className="credit-detail-block installment-detail-block">
                ...
                <div className="credit-payment-list">
                  {paymentRows(item).map(row => <article className={row.paid ? 'paid' : ''} key={row.key}>
                    ...
                    {!isOwner && item.status === 'Disetujui' && (isMarketing || isAdmin) && <button type="button" disabled={Boolean(row.paid)} onClick={() => {setPaymentTarget({item, row}); setPaymentMethod(''); setPaymentProof(null)}}><Banknote/>{row.paid ? 'Lunas' : 'Bayar Kredit'}</button>}
                  </article>)}
                </div>
              </div>}
            </section>}
