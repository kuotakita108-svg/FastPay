...
export default function CheckoutPage() {
  ...
  if (result) {
    ...
    <section className={`checkout-sheet checkout-result-sheet ${failed?'is-refunded':success?'is-success':'is-pending'}`}>
      ...
      <div className="checkout-result-notice"><i>{failed?<QrCode/>:success?<Check/>:<Clock3/>}</i><strong>{expired?'Invoice kedaluwarsa':unpaid?'Menunggu pembayaran':failed?(directH2H?'Transaksi ditolak H2HR':'Dana dikembalikan ke saldo'):success?'Pembayaran berhasil':'Transaksi sedang diproses'}</strong><p>{expired?'Batas pembayaran telah berakhir. Tidak ada saldo yang dipotong dan produk tidak dikirim ke P24.':unpaid?'Invoice tersimpan. QRIS belum tersedia, jadi tidak ada saldo yang dipotong dan produk belum dikirim ke P24.':failed?(directH2H?`${result.message||'Produk ditolak provider.'} Deposit H2H Owner tidak dipotong.`:(result.message||'Transaksi gagal di provider dan dana sudah dikembalikan ke saldo akun Anda.')):success?`${state.product} untuk ${state.target} berhasil diproses.`:'Pesanan sudah diterima Pulsa24Jam. Status final diperbarui melalui callback resmi.'}</p></div>
      <dl className="checkout-result-details">
        ...
        <div><dt>Status</dt><dd className={failed||expired?'refund-text':''}>{failed?(directH2H?'Deposit tidak dipotong':'Dana dikembalikan'):result.transaction.status}</dd></div>
