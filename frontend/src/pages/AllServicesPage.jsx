...
import {ArrowLeft,Search,ChevronRight,Sparkles,ShieldCheck,X} from 'lucide-react'
...
export default function AllServicesPage(){
  ...
  return <main className="mobile-app services-page">
    ...
    <section className="all-services modern-all-services">
      ...
      {visibleGroups.map((group,index)=><section className={`service-group tone-${index%5}`} key={group}><header><div><span>{group}</span><small>{filtered.filter(item=>item[3]===group).length} layanan</small></div></header><div className="service-group-grid">{filtered.filter(item=>item[3]===group).map(([label,type,Icon])=><button type="button" key={label} onClick={()=>open(type)}><i><Icon/></i><div><strong>{label}</strong><small>{serviceConfig[type]?.title||`Layanan ${label}`}</small></div><ChevronRight/></button>)}</div></section>)}
