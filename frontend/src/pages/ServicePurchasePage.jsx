...
export default function ServicePurchasePage(){
 ...
 const availableProviders=useMemo(()=>{const remote=serviceProducts.map(item=>item.operator),seen=new Set(),source=productsLoading?config.providers:remote;const nonGameProviders=new Set(['telkomsel','k-vision','nex parabola','transvision']);return source.filter(name=>{const key=normalize(name);if(!key||key==='garena'||key==='xl/axis'||(type==='game'&&nonGameProviders.has(key))||(type==='tv'&&key==='tv berlangganan')||seen.has(key))return false;seen.add(key);return true})},[config.providers,serviceProducts,type,productsLoading])
