...
export default function ServicePurchasePage(){
 ...
 const availableProviders=useMemo(()=>{const remote=serviceProducts.map(item=>item.operator),seen=new Set();return [...config.providers,...remote].filter(name=>{const key=normalize(name);if(!key||key==='garena'||seen.has(key))return false;seen.add(key);return true})},[config.providers,serviceProducts])
