...
export default function ServicePurchasePage(){
 ...
 const availableProviders=useMemo(()=>{const remote=serviceProducts.map(item=>item.operator),seen=new Set(),source=productsLoading?config.providers:remote;return source.filter(name=>{const key=normalize(name);if(!key||key==='garena'||(type==='tv'&&key==='tv berlangganan')||seen.has(key))return false;seen.add(key);return true})},[config.providers,serviceProducts,type,productsLoading])
