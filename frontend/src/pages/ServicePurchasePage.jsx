...
export default function ServicePurchasePage(){
 ...
 const {data=[],loading:productsLoading,error:productsError}=useAsync(loadProducts)
 ...
 if(catalog)return <main className={`mobile-app product-catalog-page service-${type} catalog-provider-${providerIndex}`}>
  ...
  {amount>0&&<footer className="catalog-payment"><div><small>Total pembayaran</small><strong>{rupiah(amount)}</strong></div><button onClick={checkout}>Lanjut Pembayaran <ChevronRight/></button></footer>}
  <MobileNav/>
 </main>
 return <main className={`mobile-app modern-purchase service-${type}`}>
  ...
  <section className="modern-purchase-body">
   ...
   {type==='pln'&&plnMode==='bill'&&plnBill&&<section className="pln-bill-card"><header><div><BadgeCheck/><span>Tagihan ditemukan dari Pulsa24Jam</span></div><strong>{rupiah(plnBill.total)}</strong></header><div className="pln-bill-owner"><ProviderLogo name="PLN"/><div><b>{plnBill.data?.customer_name||plnBill.data?.customer||'Nama pelanggan tidak dikirim provider'}</b><small>IDPEL {plnBill.idpel}</small></div></div><dl><div><dt>Nominal tagihan</dt><dd>{rupiah(plnBill.total)}</dd></div><div><dt>Status inquiry</dt><dd>{plnBill.status||'pending'}</dd></div><div><dt>Ref cek</dt><dd>{plnBill.refid}</dd></div></dl><p className="contact-hint">Pembayaran tagihan belum dibuka sampai nominal hasil inquiry diverifikasi ulang oleh server. Saldo tidak dipotong.</p></section>}
   {!(type==='pln'&&plnMode==='bill')&&<section className="provider-panel"><header><div><strong>{type==='pln'?'Pilih Produk PLN':'Pilih Penyedia'}</strong><small>{type==='pln'?'Token listrik resmi PLN':`${availableProviders.length} penyedia H2HR tersedia`}</small></div>{provider&&<span>Terpilih</span>}</header>{productsError&&<p className="contact-hint">Katalog H2HR belum dapat dimuat. Coba buka kembali halaman ini.</p>}{!productsLoading&&!productsError&&availableProviders.length===0&&<p className="contact-hint">Belum ada SKU aktif H2HR untuk layanan ini.</p>}{availableProviders.length>9&&<label className="provider-search"><Search/><input value={providerQuery} onChange={event=>{setProviderQuery(event.target.value);setProviderLimit(18)}} placeholder="Cari penyedia"/><span>{matchingProviders.length}</span></label>}<div className="modern-provider-grid">{visibleProviders.map((name,index)=><button className={`${provider===name?'active':''} tone-${index%5}`} onClick={()=>{setProvider(name);closeCatalog();setSelected(null)}} key={name}><ProviderLogo name={name} service={type} priority={index<9}/><span>{name}</span>{provider===name&&<CheckCircle2/>}</button>)}</div>{visibleProviders.length<matchingProviders.length&&<button type="button" className="provider-load-more" onClick={()=>setProviderLimit(limit=>limit+18)}>Tampilkan penyedia lainnya <ChevronRight/></button>}<button className="show-products" onClick={()=>{openCatalog();window.scrollTo({top:0,behavior:'smooth'})}} disabled={target.length<4||!provider}>Lihat Produk & Nominal <ChevronRight/></button></section>}
