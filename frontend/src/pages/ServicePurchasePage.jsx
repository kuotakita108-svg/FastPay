...
export default function ServicePurchasePage(){
 ...
 const products=useMemo(()=>{const seen=new Set();return serviceProducts.filter(product=>normalize(product.operator)===normalize(provider)&&product.sku).filter(product=>{const key=`sku:${normalize(product.sku)}`;if(seen.has(key))return false;seen.add(key);return true})},[serviceProducts,provider])
