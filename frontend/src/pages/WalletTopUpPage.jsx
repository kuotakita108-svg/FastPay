...
export default function WalletTopUpPage(){
 const navigate=useNavigate(),{user,setBalance}=useAuth(),{show}=useToast(),[amount,setAmount]=useState(100000),[custom,setCustom]=useState(''),[method,setMethod]=useState('va'),[success,setSuccess]=useState(false),[saving,setSaving]=useState(false)
 ...
 const submit=async()=>{if(value<10000){show('Minimal isi saldo Rp10.000');return}setSaving(true);try{const result=await request('/me/topups',{method:'POST',body:JSON.stringify({amount:value,method:selected?.title||'Isi Saldo'})});setBalance(result.balance);setSuccess(true)}catch(error){show(error.message||'Isi saldo belum dapat diproses')}finally{setSaving(false)}}
