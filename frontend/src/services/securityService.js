...
const cache=new Map(),bytes=value=>Array.from(new Uint8Array(value)),encode=value=>btoa(String.fromCharCode(...new Uint8Array(value))),decode=value=>Uint8Array.from(atob(value),character=>character.charCodeAt(0))
const digest=async value=>encode(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(value)))
export const getSecurity=userId=>cache.get(userId)||{}
async function save(userId,value){const result=await request('/me/preferences',{method:'PUT',body:JSON.stringify({security:value})});const saved=result.security||value;cache.set(userId,saved);return saved}
export async function loadSecurity(userId){const result=await request('/me/preferences');const saved=result.security||{};cache.set(userId,saved);return saved}
...
export async function verifyBiometric(userId){const settings=getSecurity(userId);if(!settings.biometricEnabled||!settings.biometricId)throw new Error('Sidik jari belum diaktifkan.');const assertion=await navigator.credentials.get({publicKey:{challenge:crypto.getRandomValues(new Uint8Array(32)),allowCredentials:[{id:decode(settings.biometricId),type:'public-key',transports:['internal']}],userVerification:'required',timeout:60000}});return Boolean(assertion&&bytes(assertion.rawId).length)}
