...
export default function PaymentSecurityModal({open,user,settings,onClose,onVerified}){
 ...
 if(!open)return null
 const pinSubmit=async event=>{event.preventDefault();setChecking(true);setError('');try{if(!await verifyPin(user.id,pin))throw new Error('PIN FastPay salah.');onVerified()}catch(current){setError(current.message)}finally{setChecking(false)}}
